...
exports.run = async (client, message, args) => {
       ...
       let uid = user.id // let uid = message.mentions.users.first() || client.users.cache.get(args[0]) || message.author; = x !
        ...
        let receive = ''
        let banner = `**${user.tag}** doesn't have banner.`
        ...
        banner = `**${user.tag}**'s banner' https://cdn.discordapp.com/banners/${uid}/${receive}.gif?size=1024`
        if(statut === 415) {
        banner = `**${user.tag}**'s banner https://cdn.discordapp.com/banners/${uid}/${receive}.png?size=1024`
        }
